...
public class BannerGroupEntry : ObservableObject {
    #region Delegates
    ...
    public void AddIcons(IEnumerable<string> filePaths) {
        bool IsIconAdded(BannerIconEntry icon, string filePath) =>
            icon.TexturePath.Equals(filePath, StringComparison.InvariantCultureIgnoreCase);
        ...
        IEnumerable<BannerIconEntry> existingIcons =
            Icons.Where(icon => filePaths.Any(filePath => IsIconAdded(icon, filePath)));
