...
public partial class BannerIconGroupEditorViewModel : ObservableObject {
    ...
    [ObservableProperty] private BannerGroupEntry? groupData;
    ...
    public async Task AddTextures() {
        if (_fileDialog == null || GroupData == null) {
            return;
        }

        IReadOnlyList<string>? files = await _fileDialog.OpenFiles(GUID_TEXTURE_DIALOG, new[] { CommonFileTypes.Png });
    ...
    private void OnSelectedIconPropertyChanged(object? sender, PropertyChangedEventArgs e) {
        if (e.PropertyName == nameof(BannerIconEntry.SpritePath) ||
            e.PropertyName == nameof(BannerIconEntry.TexturePath)) {
