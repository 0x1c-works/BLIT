...
public sealed partial class BannerIconGroupEditor : UserControl, INotifyPropertyChanged {
    ...
    private IEnumerable<BannerIconEntry> SelectedIcons => gridIcons.SelectedItems.Cast<BannerIconEntry>();
    private bool HasSelectedIcons => gridIcons.SelectedItems.Any();
    private BannerIconEntry FirstSelectedIcon => SelectedIcons.FirstOrDefault();
    private bool CanReimportSprite => HasSelectedIcons && ImageHelper.IsValidImage(FirstSelectedIcon.SpritePath);
    private bool CanReimportTexture => HasSelectedIcons && ImageHelper.IsValidImage(FirstSelectedIcon.TexturePath);
    ...
    private async void btnOpenTextures_Click(object sender, RoutedEventArgs e) {
        IReadOnlyList<StorageFile> files = await AppServices.Get<IFileDialogService>()
            .OpenFiles(GUID_TEXTURE_DIALOG, new[] { CommonFileTypes.Png });
    ...
    private async void btnSelectSprite_Click(object sender, RoutedEventArgs e) {
        StorageFile file = await AppServices.Get<IFileDialogService>().OpenFile(GUID_SPRITE_DIALOG,
    ...
    private async void btnSelectTexture_Click(object sender, RoutedEventArgs e) {
        StorageFile file = await AppServices.Get<IFileDialogService>().OpenFile(GUID_TEXTURE_DIALOG,
